...
export class BaseService<T, Y> {
    ...
    readPaginationSearch(
        ...
        return this.http.get<Y>(url).pipe(
    ...
    create(objeto: T): Observable<T> {
        return this.http.post<T>(`${this.baseUrl}`, objeto).pipe(
            map((response) => response),
            catchError((e: any) => {
    ...
    update(objeto: T, id: number): Observable<T> {
        const url = `${this.baseUrl}/${id}`;
        return this.http.put<T>(url, objeto).pipe(
            map((response) => response),
            catchError((e: any) => {
    ...
    errorHandler(e: any, title: string): Observable<any> {
        this.openErrorDialog(e, title);
        return throwError(e);
    }

    openErrorDialog(error: any, title: string) {
